use amethyst::{
    ...
    prelude::*,
    renderer::{Camera, ImageFormat, SpriteRender, SpriteSheet, SpriteSheetFormat, Texture, resources::Tint, Transparent, palette::Srgba},
    ui::{
...
impl SimpleState for MyState {
    ...
    fn on_start(&mut self, data: StateData<'_, GameData<'_, '_>>) {
        ...
        create_ui_example(world, &dimensions);
    }
...
fn init_sprites(world: &mut World, sprites: &[SpriteRender], dimensions: &ScreenDimensions) {
    for (i, sprite) in sprites.iter().enumerate() {
        ...
        let x = (i as f32 - 1.) * 100. + dimensions.width() * 0.25;
        ...
        world
            ...
            .with(sprite.clone())
            .with(transform)
            .with(Tint(Srgba::new(1.0, 1.0, 1.0, 0.001)))
            .with(Transparent)
            .build();
    }
    ...
    for (i, sprite) in sprites.iter().enumerate() {
        let x = (i as f32 - 1.) * 100. + dimensions.width() * 0.75;
        let y = (i as f32 - 1.) * 100. + dimensions.height() * 0.5;
        let mut transform = Transform::default();
        transform.set_translation_xyz(x, y, 0.);
...
pub fn create_ui_example(world: &mut World, dimensions: &ScreenDimensions) {
    ...
    let x = dimensions.width() * 0.25;
    let y = dimensions.height() * 0.15;
    world
        .create_entity()
        .with(UiTransform::new(
            "".to_string(),
        ...
        world
            ...
            .with(UiText::new(
                font,
                "Alpha and RGB in sync".to_string(),
                [1., 1., 1., 1.],
                30.,
                LineMode::Single,
                Anchor::TopLeft,
